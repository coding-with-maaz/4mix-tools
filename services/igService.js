...
exports.getVideoInfo = async (url) => {
  ...
  if (!/instagram\.com/.test(url)) {
    throw new Error('Not a valid Instagram video URL');
  }
  ...
  return {
    platform: data.extractor_key, // Should be 'Instagram'
    title: data.title,
...
exports.downloadVideo = async (url, format, res) => {
  ...
  ytDlp.stderr.on('data', (data) => {
    ...
    if (progressMatch) {
      ...
      process.stdout.write(`\rInstagram Download Progress: ${percent}%   `);
    }
  });
